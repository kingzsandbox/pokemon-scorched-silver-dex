...
import { getLocationById, getLocationGroupByLocationId, getLocationPageHref } from "./locations";
...
export function formatEncounterRate(encounterRate: number): string {
  const percent = encounterRate > 0 && encounterRate < 1 ? encounterRate * 100 : encounterRate;
...
export function getEncounterRowsByPokemonId(pokemonId: string): PokemonEncounterRow[] {
  return (encountersByPokemon.get(pokemonId) ?? [])
    .map((entry) => {
      ...
      const locationGroup = getLocationGroupByLocationId(entry.locationId);

      return {
        encounterId: entry.id,
        locationName: locationGroup?.name ?? location.name,
        locationSlug: (locationGroup ? getLocationPageHref(location) : `/locations/${location.slug}`).replace(/^\/locations\//, ""),
        method: entry.method,
