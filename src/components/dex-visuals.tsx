...
function getEncounterVisualMeta(method: string): EncounterVisualMeta {
  ...
  if (normalized === "fishing - old rod") {
    return { label: "Fishing - Old Rod", kind: "fishing" };
  }
  if (normalized === "fishing - good rod") {
    return { label: "Fishing - Good Rod", kind: "fishing" };
  }
  if (normalized === "fishing - super rod") {
    return { label: "Fishing - Super Rod", kind: "fishing" };
  }
  if (normalized === "surf") {
    return { label: "Surf", kind: "surf" };
  }
  if (normalized === "dive / underwater" || normalized === "underwater") {
    return { label: "Dive / Underwater", kind: "surf" };
  }
  if (normalized === "rock smash" || normalized === "rock_smash") {
    return { label: "Rock Smash", kind: "rock-smash" };
  }
  if (normalized === "fishing") {
    return { label: "Fishing", kind: "fishing" };
  }
  if (normalized === "horde") {
...
export function matchupSectionStyle(
  ...
  if (multiplier === 0) {
    return { background: "var(--matchup-immune-bg)", color: "var(--matchup-immune-text)", border: "1px solid var(--border-soft)" };
  }

  if (mode === "defensive") {
    if (multiplier >= 2) {
      return { background: "var(--matchup-bad-bg)", color: "var(--matchup-bad-text)", border: "1px solid var(--border-soft)" };
    }
    if (multiplier <= 0.25) {
      return { background: "var(--matchup-great-bg)", color: "var(--matchup-great-text)", border: "1px solid var(--border-soft)" };
    }
    if (multiplier < 1) {
      return { background: "var(--matchup-good-bg)", color: "var(--matchup-good-text)", border: "1px solid var(--border-soft)" };
    }
    return { background: "var(--surface-muted)", color: "var(--text-body)", border: "1px solid var(--border-soft)" };
  }
